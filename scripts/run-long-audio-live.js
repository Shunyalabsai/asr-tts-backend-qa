...
const ASR_BASE_URL = process.env.ASR_BASE_URL || 'https://asrv2prod.shunyalabs.ai';
...
async function getJwtToken() {
  const url = `${ASR_BASE_URL}/auth/token`;
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to get JWT token [${res.status}]: ${text}`);
  }
...
async function run() {
  console.log('=== EXECUTING ALL 7 LONG AUDIO TEST CASES (LIVE ASR PRODUCTION) ===\n');
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!fs.existsSync(audioPath)) {
      ...
      results.push({ testId, audio: path.basename(rawAudioPath), status: 'SKIPPED', error: 'File not found' });
      continue;
    }
    ...
    try {
      const response = await fetch(`${ASR_BASE_URL}/v1/audio/transcriptions`, {
        method: 'POST',
        headers: {
      ...
      const predText = data.text || data.transcript || data.transcribed_text || '';
      const wer = calculateWER(gtText, predText);
      const cer = calculateCER(gtText, predText);
      const pass = wer <= 0.35 || cer <= 0.35;

      console.log(`  ✓ HTTP ${response.status} | Latency: ${latency} | Transcribed: ${predText.length} chars / ${predText.split(/\s+/).length} words`);
      console.log(`  WER: ${(wer * 100).toFixed(1)}% | CER: ${(cer * 100).toFixed(1)}% | Status: ${pass ? 'PASS ✅' : 'FAIL ❌'}`);
      console.log(`  GT Sample:   ${JSON.stringify(gtText.substring(0, 70))}`);
      console.log(`  Pred Sample: ${JSON.stringify(predText.substring(0, 70))}\n`);

      results.push({
        testId,
        audio: path.basename(audioPath).substring(0, 45) + '...',
        latency,
      ...
      console.error(`  ❌ Error executing ${testId}:`, err.message);
      results.push({ testId, audio: path.basename(audioPath), status: 'ERROR ❌', error: err.message });
    }
  }

  console.log('═══════════════════════════════════════════════════════════════════════════════════════════');
  console.log('                           LONG AUDIO TEST SUITE SUMMARY                                   ');
  console.log('═══════════════════════════════════════════════════════════════════════════════════════════');
