...
<div class="tab-content" id="testcasesTab">
  <div class="test-explorer-card">
    ...
    <div class="search-controls">
      ...
      <select id="runSelectFilter" class="select-ctl" onchange="onRunSelectChange()">
        <!-- Populated dynamically with all historical runs -->
      </select>
...
let currentSelectedMatrixRun = latestData;
...
function populateRunSelector() {
  const sel = document.getElementById('runSelectFilter');
  if (!sel) return;
  sel.innerHTML = historyData.map((r, idx) => {
    const isLatest = idx === 0;
    const dateLabel = formatDate(r.startedAt);
    const timeLabel = formatTime(r.startedAt);
    const label = \`\${isLatest ? '★ Latest: ' : ''}\${dateLabel} (\${timeLabel}) — \${r.summary ? r.summary.total : (r.tests ? r.tests.length : 0)} Tests [\${r.passRate}%]\`;
    return \`<option value="\${r.id}" \${isLatest ? 'selected' : ''}>\${label}</option>\`;
  }).join('');
}
...
function renderAllTestCasesTable(tests) {
  ...
  if (countBadge) {
    const totalPool = (currentSelectedMatrixRun && currentSelectedMatrixRun.tests) ? currentSelectedMatrixRun.tests.length : latestData.tests.length;
    countBadge.textContent = \`Showing \${tests.length} of \${totalPool}\`;
  }
}
...
function filterTestCasesTable() {
  ...
  const currentPool = (currentSelectedMatrixRun && currentSelectedMatrixRun.tests && currentSelectedMatrixRun.tests.length > 0)
    ? currentSelectedMatrixRun.tests
    : latestData.tests;

  const filtered = currentPool.filter(t => {
    const isSmoke = t.priority === 'P0' || t.suite === 'Core System' || t.module === 'Core-System-Tests';
    const matchesQuery =
...
function openTestModalByTest(testId, suite, moduleName, title) {
  ...
  if (!t) {
    t = testsPool.find(item => item.id === testId && (item.module === moduleName || item.suite === suite));
  }
  if (!t) {
    t = testsPool.find(item => item.id === testId || item.id.endsWith(':' + testId));
  }
  if (!t && latestData.tests) {
    t = latestData.tests.find(item => item.id === testId && item.title === title) || latestData.tests.find(item => item.id === testId);
  }
  if (!t) return;

  renderModalContentForTest(t);
}

function openTestModalDirectly(testId) {
  openTestModalByTest(testId, '', '', '');
}

function renderModalContentForTest(t) {
  const isSmoke = t.priority === 'P0' || t.suite === 'Core System' || t.module === 'Core-System-Tests';
  const body = \`
    <div style="display:flex;flex-direction:column;gap:14px">
      <div>
        ...
        &middot; <strong style="color:#fff">\${t.moduleLabel || t.module}</strong> &middot; <span style="color:#c4b5fd">\${t.feature}</span>
      </div>
      <div>
